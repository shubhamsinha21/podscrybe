...
import { groq } from "@/lib/groq-client"; // your new Groq client
...
export async function generateHashtags(
  ...
  console.log("Generating hashtags with GROQ");

  try {
    // bind Groq API method so step.ai.wrap keeps context
    const createCompletion = groq.chat.completions.create.bind(
      groq.chat.completions
    );

    // Execute Groq request wrapped with Inngest observability
    const response = (await step.ai.wrap(
      "generate-hashtags-with-groq",
      createCompletion,
      {
    ...
    console.error("GROQ hashtag generation error:", error);
